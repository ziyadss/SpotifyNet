...
{
    public Task<Artist> GetArtist(string artistId, string accessToken, CancellationToken cancellationToken)
    {
        var uri = Endpoints.GetArtist(artistId);

        return _webAPIClient.GetAsync<Artist>(uri, accessToken, cancellationToken);
    }
