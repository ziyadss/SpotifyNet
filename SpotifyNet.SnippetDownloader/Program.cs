...
internal sealed class Program
{
    private static readonly string[] _requiredScopes =
        { AuthorizationScope.UserLibraryRead, AuthorizationScope.PlaylistReadPrivate };
    ...
    public static Task Main(string[] args)
    {
        ...
        builder.ConfigureServices(services => services.AddSpotifyNetServices().AddSnippetDownloader());
        ...
        ISnippetDownloader snippetDownloader)
    {
        ...
        var outputDirectoryOption =
            new Option<DirectoryInfo>("--output-directory", "The directory to place the downloader's outputs.")
            {
                IsRequired = true,
            };

        var newTokenOption = new Option<bool>("--force-generate-token", () => false,
                                              "Whether to force the generation of a new access token.")
        {
            IsRequired = false,
        };

        var globalOptions = new Option[]
            { appClientIdOption, appRedirectUriOption, outputDirectoryOption, newTokenOption };
        ...
        trackCommand.SetHandler(
            async (trackIds, outputDirectory, newToken) =>
                await DownloadTracks(newToken, trackIds, outputDirectory.FullName, tokenAcquirerService,
                                     snippetDownloader), trackIdsOption, outputDirectoryOption, newTokenOption);
        ...
        playlistCommand.SetHandler(
            async (playlistId, outputDirectory, newToken) =>
                await DownloadPlaylist(newToken, playlistId, outputDirectory.FullName, tokenAcquirerService,
                                       snippetDownloader), playlistIdOption, outputDirectoryOption, newTokenOption);
    ...
    private static async Task WriteOutput(string outputDirectory, IReadOnlyList<SnippetDownloadMetadata> output)
    {
